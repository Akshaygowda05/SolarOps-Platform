...
chripstackRouter.post('/devices/:deviceId/queue', authenticate,async (req: Request, res: Response,next: NextFunction) => {
    try {
        ...
        const now = new Date();
        const { data, name } = req.body;  // the data should be base64 formate
...
chripstackRouter.get('/ApplicationGateways',authenticate,async(req: Request,res: Response)=>{
    try {
       ...
       // console.log("Application ID from request:", applicationId); // Debug log to check the application ID
        ...
        if(!tenantRes){
            throw new AppError('Tenant not found for the application',StatusCodes.NOT_FOUND);
}
  ...
  //console.log("Tenant ID for the application:", teneantId); // Debug log to check the tenant ID
        ...
        let gatewayData =gatewayResponse.data.result;

        const filteredGatewayData = configGateway.length === 0 ? gatewayData : gatewayData.filter((gateway: any) => configGateway.includes(gateway.gatewayId));
