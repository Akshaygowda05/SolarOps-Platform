    ...
    static async createNewData(
        ...
        try {
            ...
            if (previousOdometer !== undefined) {
                //  if we have a previous odometer, calculate distance traveled
                ...
                // if no previous odometer, attempt to calculate panels cleaned based on total odometer
                // first odometer we will keep always 0 because if robot already cleaned some panels before we start tracking, we don't want to count those panels as cleaned by us
                // if we calculate also we will get huge value which   but next day data will very less
                // if i do somethign like this then proper data will be there and also i can see graph will be good in the dashboard
                panelsCleaned = 0;
            }
