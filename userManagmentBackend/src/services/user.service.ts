  ...
  try {
    ...
    static async deletUser(id:number):Promise<void>{
        ...
        const user =   await prisma.user.findUnique({
           ...
           loggers.error('Error deleting user:', error);
            throw error;
        }
    }
    ...
    static async userLogin(email:string,password:string){
        try {
            ...
            if(!user || !user.isActive){
                throw new AppError(`Invalid email please try again `, StatusCodes.UNAUTHORIZED);
            }
           ...
           }, envconfig.getTokenSecret(), { expiresIn: '1day' });
           ...
           return {
            ...
            loggers.error('Error during user login:', error);
            throw error;
        }
    }
    ...
    static async updateuserPassword(userid:number,newPassword:string):Promise<void>{
        try {
            ...
            loggers.error('Error updating user password:', error);
            throw error;
        }
    }
